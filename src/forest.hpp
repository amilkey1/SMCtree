...
//    inline double Forest::calcTransitionProbabilityLazy(Node* child, double s, double s_child, unsigned locus, double edgelen_extension) {
//        double relative_rate = G::_double_relative_rates[locus];
//        assert (relative_rate > 0.0);
//
//        double child_transition_prob = 0.0;
//
//        if (G::_model_type == G::ModelType::MODEL_TYPE_JC) {
//            double expterm = exp(-4.0*((child->_edge_length + edgelen_extension) * _clock_rate * relative_rate)/3.0);
//            double prsame = 0.25+0.75*expterm;
//            double prdif = 0.25 - 0.25*expterm;
//
//            child_transition_prob = (s == s_child ? prsame : prdif);
...
//
//            double phi = (pi_A+pi_G)*(pi_C+pi_T)+G::_kappa*(pi_A*pi_G+pi_C*pi_T);
//            double beta_t = 0.5*((child->_edge_length + edgelen_extension) * _clock_rate * relative_rate)/phi;
//
//            // transition prob depends only on ending state
//            if (s_child == 0) {
//                // purine
